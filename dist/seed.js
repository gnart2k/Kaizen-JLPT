...
async function seedLanguages() {
    ...
    try {
        const result = await db_1.db
            ...
            .onConflictDoNothing({
            target: [schema_1.languages.code],
        })
            .returning({ id: schema_1.languages.id, code: schema_1.languages.code });
...
async function seedDifficultyLevels() {
    ...
    try {
        ...
        const languageMap = await db_1.db.query.languages.findMany({
            columns: { id: true, code: true }
        }).then(languages => languages.reduce((acc, lang) => {
        ...
        const result = await db_1.db
            .insert(schema_1.difficultyLevels)
            .values(seedData)
            .onConflictDoNothing({
            target: [schema_1.difficultyLevels.languageId, schema_1.difficultyLevels.levelName],
        })
            .returning({ id: schema_1.difficultyLevels.id });
